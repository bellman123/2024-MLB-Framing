...
best_sum_24 <- best_sum_24[,c(1,7,8,2:6)]

best_sum_24 <- best_sum_24 %>% arrange(desc(frame_z))
...
summarize(tot_frames = sum(ifelse((frame_class == 'nm_frame' | frame_class == 'robbery' | frame_class == 'frame'),1,0)),
tot_misses = sum(ifelse(frame_class == 'miss',1,0)),
exp_run_added = round(sum(edit_run_exp_change_3, na.rm = TRUE), digits=2),
frames_per_miss = round(tot_frames/tot_misses, digits=2),
frame_perc = round(100*(sum(ifelse((frame_class == 'nm_frame' | frame_class == 'robbery' | frame_class == 'frame'),1,0))/n()), digits=2),
miss_perc = round(100*sum(ifelse(frame_class == 'miss',1,0))/sum(ifelse((frame_class == 'strike' | frame_class == 'frame' | frame_class == 'miss'),1,0)), digits=2),
exp_run_added_per_pitch = sum(edit_run_exp_change_3, na.rm = TRUE)/n(),
total_pitches = n(),
delta_run_exp = sum(delta_run_exp_edit, na.rm=TRUE),
delta_run_exp_per_pitch = sum(delta_run_exp_edit, na.rm=TRUE)/n())
...
matrix_outs_0 <- generate_matrix(count_out_base_innV2, 0, base_order) 
matrix_outs_1 <- generate_matrix(count_out_base_innV2, 1, base_order) 
matrix_outs_2 <- generate_matrix(count_out_base_innV2, 2, base_order)
...
visualize_matrix <- function(matrix_data, title) { 
  ...
  geom_tile() + 
  geom_text(aes(label = round(exp_runs_per_inn, 2))) + 
...
#visualize run expectancy
count_out_base_innV2 %>%
mutate(count_out = substr(count_out_base,1,5),
count = substr(count_out_base,1,3),
out = substr(count_out_base,5,5),
base_state = sapply(strsplit(count_out_base_innV2$count_out_base, ','), function(x) x[3])) %>%
mutate(count_base = paste(count, base_state, sep = ',')) %>%
ggplot(aes(x=count_out, y=exp_runs_per_inn)) +
geom_point(aes(color = base_state)) +
#geom_line(aes(color = count_base)) +
theme(axis.text.x = element_text(angle = 45, hjust=1))

#exp runs PER AB analysis
#out-baserunner and out-count-baserunner expected run matrices PER AB
out_base_ab <- plays_24 %>%
group_by(out_base) %>%
summarize(exp_runs_per_ab = sum(runs_scored)/n(), runs = sum(runs_scored),
n = n()) %>%
arrange(exp_runs_per_ab)

count_out_base_ab <- plays_24 %>%
group_by(count_out_base) %>%
summarize(exp_runs_per_ab = sum(runs_scored)/n(), runs = sum(runs_scored),
n = n()) %>%
arrange(exp_runs_per_ab) %>%
filter(substr(count_out_base,1,1) != '4')

#plot of out-baserunner state expected runs PER AB
plays_24 %>%
group_by(out_base) %>%
summarize(exp_runs_per_ab = sum(runs_scored)/n(), runs = sum(runs_scored),
n = n()) %>%
arrange(exp_runs_per_ab) %>%
ggplot(aes(x=exp_runs_per_ab,y= fct_reorder(out_base,exp_runs_per_ab))) +
geom_col(fill = 'maroon') +
geom_label(aes(label = round(exp_runs_per_ab,digits=2)), size = 3, nudge_y = -0.025) +
labs(x = 'Expected runs per AB',
y= 'Out, baserunner state',
title = 'Expected runs for each out-baserunner state in 2024 MLB games')

#plot of expected runs by count PER AB
plays_24 %>%
group_by(count) %>%
summarize(exp_runs_per_ab = sum(runs_scored)/n(), runs = sum(runs_scored),
n = n()) %>%
arrange(exp_runs_per_ab) %>%
ggplot(aes(x=exp_runs_per_ab,y= fct_reorder(count,exp_runs_per_ab))) +
geom_col(fill = 'navy') +
geom_label(aes(label = round(exp_runs_per_ab,digits=2)), size = 3, nudge_y = -0.025) +
labs(x = 'Expected runs per AB',
y= 'At-bat count',
